...
func (d *Database) createTables() error {
	...
	alterQueries := []string{
		...
		`ALTER TABLE traders ADD COLUMN thinking_level TEXT DEFAULT ''`,                // Gemini Thinking Level
...
type TraderRecord struct {
	...
	ThinkingLevel        string    `json:"thinking_level"`         // Gemini Thinking Level (low/high)
	CreatedAt            time.Time `json:"created_at"`
	UpdatedAt            time.Time `json:"updated_at"`
}
...
func (d *Database) CreateTrader(trader *TraderRecord) error {
	_, err := d.db.Exec(`
		INSERT INTO traders (id, user_id, name, ai_model_id, exchange_id, initial_balance, scan_interval_minutes, is_running, btc_eth_leverage, altcoin_leverage, trading_symbols, use_coin_pool, use_oi_top, custom_prompt, override_base_prompt, system_prompt_template, is_cross_margin, thinking_level)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, trader.ID, trader.UserID, trader.Name, trader.AIModelID, trader.ExchangeID, trader.InitialBalance, trader.ScanIntervalMinutes, trader.IsRunning, trader.BTCETHLeverage, trader.AltcoinLeverage, trader.TradingSymbols, trader.UseCoinPool, trader.UseOITop, trader.CustomPrompt, trader.OverrideBasePrompt, trader.SystemPromptTemplate, trader.IsCrossMargin, trader.ThinkingLevel)
	return err
}
...
func (d *Database) GetTraders(userID string) ([]*TraderRecord, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&trader.IsCrossMargin, &trader.ThinkingLevel,
...
func (d *Database) UpdateTrader(trader *TraderRecord) error {
	_, err := d.db.Exec(`
		UPDATE traders SET
			...
			system_prompt_template = ?, is_cross_margin = ?, thinking_level = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ? AND user_id = ?
	`, trader.Name, trader.AIModelID, trader.ExchangeID, trader.InitialBalance,
		trader.ScanIntervalMinutes, trader.BTCETHLeverage, trader.AltcoinLeverage,
		trader.TradingSymbols, trader.CustomPrompt, trader.OverrideBasePrompt,
		trader.SystemPromptTemplate, trader.IsCrossMargin, trader.ThinkingLevel, trader.ID, trader.UserID)
	return err
}
...
func (d *Database) GetTraderConfig(userID, traderID string) (*TraderRecord, *AIModelConfig, *ExchangeConfig, error) {
	...
	err := d.db.QueryRow(`
		SELECT
			...
			COALESCE(t.thinking_level, '') as thinking_level,
